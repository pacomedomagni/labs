...
export class DeviceService {
  ...
  swapDevices(sourceParticipantSequenceId: number, destinationParticipantSequenceId: number): Observable<Resource> {
    return this.api.post<Resource>({
  ...
  getAudioStatusAWS(deviceSerialNumber: string): Observable<Resource> {
    return this.api.get<Resource>({ uri: `${this.controller}/GetAudioStatusAWS?deviceSerialNumber=${deviceSerialNumber}` });
  }

  setAudioStatusAWS(deviceSerialNumber: string, isAudioOn: boolean): Observable<Resource> {
    return this.api.patch<Resource>({ uri: `${this.controller}/SetAudioStatusAWS?deviceSerialNumber=${deviceSerialNumber}&isAudioOn=${isAudioOn}` });
  }
