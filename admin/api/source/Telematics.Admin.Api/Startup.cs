using DistributedTracingDotNetStandard20;
using MediatR;
...
using Progressive.Telematics.Admin.Api.Middleware;
...
using Progressive.Telematics.Admin.Shared.RetryPolicyProviders;
...
namespace Progressive.Telematics.Admin.Api
{
        ...
        */
        public void ConfigureServices(IServiceCollection services)
        {
            services
                ...
                .AddMediatR(typeof(IIncidentResolutionOrchestrator))
                .AddTransient<IRetryPolicyProvider, RetryPolicyProvider>();

            services.AddStandardTracing("Telematics Core Services", "TMXAdminApp", configureTracingOptions: (options) =>
            {
                options.LoggingLevel = Microsoft.Extensions.Logging.LogLevel.Information;
                options.FilterUrlOverrideHandler = GetListOfUrlsToExcludeFromTracing;
            });

            services.AddHealthChecks();
            services
                .AddControllers()
                .AddJsonOptions(x => x.JsonSerializerOptions.SetDefaultJsonSerializerOptions());

            services
                .AddHttpContextAccessor();


            string clientSecret = Environment.GetEnvironmentVariable(Configuration.GetSection("WebGuardClient").GetValue<String>("ClientSecretName"));
            string clientId = Environment.GetEnvironmentVariable(Configuration.GetSection("WebGuardClient").GetValue<String>("ClientIdName"));
            ...
            services
                ...
                .AddFeatureFlagManager(builder =>
                {
                    builder.AddFlags(typeof(RolloutHelper));
                    builder.Configure(Configuration.GetSection("FlagManagerOptions"));
                    builder.AddGlobalContext("appSubLvlNam", "TMXAdminApp");
                })
                .AddMemoryCache();
            ...
            services.AddCors(options =>
            {
                options.AddDefaultPolicy(policy =>
                {
                    policy.AllowAnyOrigin()
                          .AllowAnyMethod()
                          .AllowAnyHeader()
                          .WithExposedHeaders("x-pagination", "Server-Timing");
                });
            });
        ...
        private bool GetListOfUrlsToExcludeFromTracing(HttpContext context)
        {
            string[] urlsToExclude = { "healthz", "liveness", "refreshcache" };
            foreach (string s in urlsToExclude)
            {
                if (context.Request.Path.ToString().ToLower().Contains(s.ToLower()))
                    return false;
