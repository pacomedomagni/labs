...
{
	public abstract class ModernApiBase<T>
	{
		...
		public ModernApiBase(ILogger<T> logger, IHttpClientFactory clientFactory, ApiConfig config)
		{
			this.logger = logger;
			this.clientFactory = clientFactory;
			this.client = clientFactory.CreateClient("ModernToken");
			this.config = config;
		}
		...
		protected async Task<T1> DeserializeResponseContent<T1>(StringContent content, JsonSerializerOptions serializerSettings = null) where T1 : class
		{
        ...
        protected async Task<dynamic> UnwrapModernTelematicsExceptionMessage(HttpResponseMessage response)
        {
            try
            {
                return JsonSerializerExtensions.DeserializeAnonymousType(await response.Content.ReadAsStringAsync(), new { DeveloperMessage = "", ErrorCode = 0 }, JsonSerializerSettings.CaseInsensitive);
            }
            catch
            {
                return null;
